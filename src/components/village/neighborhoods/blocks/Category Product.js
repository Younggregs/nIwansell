...
export default class CategoryProduct extends React.Component {
  ...
  async componentWillMount() {
    ...
    if (this.props.show_more){
      try {
        const res = await fetch('http://www.iwansell.com/api/category_product/' + this.props.campus_id + '/99' + '/1');
      ...
      try {
        const res = await fetch('http://www.iwansell.com/api/category_product/' + this.props.campus_id + '/99' + '/0');
        const categoryProductList = await res.json();
        this.setState({
          categoryProductList
        });
  ...
  async newCategory(id,name) {
    ...
    try {
      const res = await fetch('http://www.iwansell.com/api/category_product/' + this.props.campus_id + '/' + id);
      const categoryProductList = await res.json();
      this.setState({
        categoryProductList
      });
