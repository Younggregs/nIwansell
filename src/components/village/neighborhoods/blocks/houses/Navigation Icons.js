...
export default class NavigationIcons extends React.Component {
  ...
  async componentWillMount() {
    ...
    const auth = localStorage.getItem('auth_code')
      ...
      try {
        const res = await fetch('http://127.0.0.1:8000/unread_messages/', {
      ...
      try {
        const res = await fetch('http://127.0.0.1:8000/have_eshop/', {
      ...
      if(this.state.have_eshop) {
        ...
        try {
          const res = await fetch('http://127.0.0.1:8000/my_eshopid/', {
